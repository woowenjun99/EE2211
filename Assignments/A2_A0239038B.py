...
def make_dataset():
    ...
    class1a = np.random.normal(loc=(0, 1), scale=(short, _long), size=(
        N_per_cluster, 2))  # scale=(x, y) = (width,height)
    class1b = np.random.normal(
        loc=(0, -1), scale=(short, _long), size=(N_per_cluster, 2))
    class1a = make_hole(class1a, 0, 1, r_hole)
    class1b = make_hole(class1b, 0, -1, r_hole)

    class2a = np.random.normal(
        loc=(-1, 0), scale=(_long, short), size=(N_per_cluster, 2))
    class2b = np.random.normal(loc=(0, 1), scale=(
        r_hole/5, r_hole/5), size=(int(N_per_cluster), 2))

    class3a = np.random.normal(loc=(1, 0), scale=(
        _long, short), size=(N_per_cluster, 2))
    class3b = np.random.normal(
        loc=(0, -1), scale=(r_hole/5, r_hole/5), size=(int(N_per_cluster), 2))

    X = np.vstack([class1a, class1b, class2a, class2b, class3a, class3b])
    Y = np.hstack([np.zeros(len(class1a)), np.zeros(len(class1b)), np.ones(len(class2a)), np.ones(
        len(class2b)),  np.ones(len(class3a))*2,  np.ones(len(class3b))*2]).astype(np.uint)
...
# DO NOT modify the parameter enable_visualization as it will be used during grading.
def A2_A0239038B(enable_visualization=True):
        ...
        try:
            ...
            for c in range(3):
                ...
                plt.scatter(X_train[indices, 0],
                            X_train[indices, 1], s=2, c=color_dict[c])
            plt.savefig('train_set.png')
            plt.figure()
            for c in range(3):
                plt.title("test set")
                indices = (Y_test == c)
                plt.scatter(X_test[indices, 0],
                            X_test[indices, 1], s=2, c=color_dict[c])
    ...
    for order in range(1, 31):
        ...
        if num_rows <= num_cols:
            wp = P_train.T @ np.linalg.inv(P_train @ P_train.T +
                                           alpha * (np.identity(num_rows))) @ Y_train_onehot
        else:
            wp = np.linalg.inv(P_train.T @ P_train + alpha *
                               (np.identity(num_cols))) @ P_train.T @ Y_train_onehot

        train_acc = np.mean(np.argmax(P_train @ wp, axis=1) == Y_train)
        ...
        if enable_visualization:
            try:
                ...
                plt.figure()
                plot_decision_boundary(lambda x: poly.fit_transform(
                    x).dot(wp).argmax(1), X=X_train)
                for c in range(3):
                    indices = (Y_train == c)
                    plt.scatter(X_train[indices, 0],
                                X_train[indices, 1], s=2, c=color_dict[c])
    ...
    print(
        f"The highest test accuracy is {best_test_acc} and the order is {best_order}")
...
if __name__ == '__main__':
    X_train, Y_train, X_test, Y_test, Y_train_onehot, Y_test_onehot, best_order, best_P_train, best_P_test, best_wp, best_train_acc, best_test_acc = A2_A0239038B(
